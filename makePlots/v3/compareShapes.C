void compareShapes() {
  ...
  /*
  TString vars[6] = {"pfMET_t01", "HT", "HT_jets", "dPhi_leadPhoton_l", "dPhi_met_l", "dPhi_met_ht"};
  ...
  for(int i = 0; i < 6; i++) {
    overlaySignalRegions(vars[i], "ele");
    overlaySignalRegions(vars[i], "muon");
  }

  overlaySignalRegions("ele_pt", "ele");
  overlaySignalRegions("muon_pt", "muon");
...
void overlayCR1SR1(TString channel) {
  ...
  for(int i = 1; i < 10; i++) {
    ...
    if(nc < 1.e-6 || n1 < 1.e-6) continue;
    if(ec > nc || e1 > n1) continue;
