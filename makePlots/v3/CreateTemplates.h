...
void TemplateMaker::FillMCBackgrounds() {
  ...
  for(unsigned int i = 0; i < mcTrees.size(); i++) {
	...
	if(Nphotons >= 2 && (leadChHadIso >= 2.6 || trailChHadIso >= 2.6)) continue;
      ...
      mcHistograms_btagWeightUp[i]->Fill(value, totalWeight * btagWeightUp/btagWeight);
      mcHistograms_btagWeightDown[i]->Fill(value, totalWeight * btagWeightDown/btagWeight);
	
      mcHistograms_puWeightUp[i]->Fill(value, totalWeight * puWeightUp/puWeight);
      mcHistograms_puWeightDown[i]->Fill(value, totalWeight * puWeightDown/puWeight);
      
      mcHistograms_leptonSFup[i]->Fill(value, totalWeight * leptonSFup/leptonSF);
      mcHistograms_leptonSFdown[i]->Fill(value, totalWeight * leptonSFdown/leptonSF);
      
      mcHistograms_photonSFup[i]->Fill(value, totalWeight * photonSFup/photonSF);
      mcHistograms_photonSFdown[i]->Fill(value, totalWeight * photonSFdown/photonSF);
      
      if(reweightTopPt[i]) {
	mcHistograms_topPtUp[i]->Fill(value, totalWeight * topPtReweighting);
	mcHistograms_topPtDown[i]->Fill(value, totalWeight / topPtReweighting);
      }
      else {
	...
	mcHistograms_matchPhoton_btagWeightUp[i]->Fill(value, totalWeight * btagWeightUp/btagWeight);
	mcHistograms_matchPhoton_btagWeightDown[i]->Fill(value, totalWeight * btagWeightDown/btagWeight);
	
	mcHistograms_matchPhoton_puWeightUp[i]->Fill(value, totalWeight * puWeightUp/puWeight);
	mcHistograms_matchPhoton_puWeightDown[i]->Fill(value, totalWeight * puWeightDown/puWeight);
	
	mcHistograms_matchPhoton_leptonSFup[i]->Fill(value, totalWeight * leptonSFup/leptonSF);
	mcHistograms_matchPhoton_leptonSFdown[i]->Fill(value, totalWeight * leptonSFdown/leptonSF);
	
	mcHistograms_matchPhoton_photonSFup[i]->Fill(value, totalWeight * photonSFup/photonSF);
	mcHistograms_matchPhoton_photonSFdown[i]->Fill(value, totalWeight * photonSFdown/photonSF);
	
	if(reweightTopPt[i]) {
	  mcHistograms_matchPhoton_topPtUp[i]->Fill(value, totalWeight * topPtReweighting);
	  mcHistograms_matchPhoton_topPtDown[i]->Fill(value, totalWeight / topPtReweighting);
	}
	else {
	  mcHistograms_matchPhoton_topPtUp[i]->Fill(value, totalWeight);
	  mcHistograms_matchPhoton_topPtDown[i]->Fill(value, totalWeight);
	}
	
      } // if match gamma

      else if(leadMatchElectron == 1) {

	FillWithError(mcHistograms_matchElectron[i], value, totalWeight, addError2);
	
	mcHistograms_matchElectron_btagWeightUp[i]->Fill(value, totalWeight * btagWeightUp/btagWeight);
	mcHistograms_matchElectron_btagWeightDown[i]->Fill(value, totalWeight * btagWeightDown/btagWeight);
	
	mcHistograms_matchElectron_puWeightUp[i]->Fill(value, totalWeight * puWeightUp/puWeight);
	mcHistograms_matchElectron_puWeightDown[i]->Fill(value, totalWeight * puWeightDown/puWeight);
	
	mcHistograms_matchElectron_leptonSFup[i]->Fill(value, totalWeight * leptonSFup/leptonSF);
	mcHistograms_matchElectron_leptonSFdown[i]->Fill(value, totalWeight * leptonSFdown/leptonSF);
	
	mcHistograms_matchElectron_photonSFup[i]->Fill(value, totalWeight * photonSFup/photonSF);
	mcHistograms_matchElectron_photonSFdown[i]->Fill(value, totalWeight * photonSFdown/photonSF);
	
	if(reweightTopPt[i]) {
	  mcHistograms_matchElectron_topPtUp[i]->Fill(value, totalWeight * topPtReweighting);
	  mcHistograms_matchElectron_topPtDown[i]->Fill(value, totalWeight / topPtReweighting);
	}
	else {
	  mcHistograms_matchElectron_topPtUp[i]->Fill(value, totalWeight);
	  mcHistograms_matchElectron_topPtDown[i]->Fill(value, totalWeight);
	}
	...
	mcHistograms_matchJet_btagWeightUp[i]->Fill(value, totalWeight * btagWeightUp/btagWeight);
	mcHistograms_matchJet_btagWeightDown[i]->Fill(value, totalWeight * btagWeightDown/btagWeight);
	
	mcHistograms_matchJet_puWeightUp[i]->Fill(value, totalWeight * puWeightUp/puWeight);
	mcHistograms_matchJet_puWeightDown[i]->Fill(value, totalWeight * puWeightDown/puWeight);
	
	mcHistograms_matchJet_leptonSFup[i]->Fill(value, totalWeight * leptonSFup/leptonSF);
	mcHistograms_matchJet_leptonSFdown[i]->Fill(value, totalWeight * leptonSFdown/leptonSF);
	
	mcHistograms_matchJet_photonSFup[i]->Fill(value, totalWeight * photonSFup/photonSF);
	mcHistograms_matchJet_photonSFdown[i]->Fill(value, totalWeight * photonSFdown/photonSF);
	
	if(reweightTopPt[i]) {
	  mcHistograms_matchJet_topPtUp[i]->Fill(value, totalWeight * topPtReweighting);
	  mcHistograms_matchJet_topPtDown[i]->Fill(value, totalWeight / topPtReweighting);
	}
	else {
	  mcHistograms_matchJet_topPtUp[i]->Fill(value, totalWeight);
	  mcHistograms_matchJet_topPtDown[i]->Fill(value, totalWeight);
	}
